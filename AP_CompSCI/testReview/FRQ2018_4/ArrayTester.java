...
public class ArrayTester
{
  ...
  {
    int[] topRow = new int[square.length];
    int[] row;
    ...
    if (containsDuplicates(topRow)) {
    	return false;
    } else {
    	for (int i = 0; i < square.length; i++) {
    		if(!hasAllValues(getColumn(square, i), topRow)) {
    			return false;
    		} else {
    			row = new int[square.length];
    			
    			for (int x = 0; x < square.length; x++) {
    				row[x] = square[x][i];
    			}
    			
    			if(!hasAllValues(row, topRow)) {
    				return false;
    			}
    		}
    	}
